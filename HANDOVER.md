...
| b | **Add a start-up check on the system's settings** | **The photo half is done, 23 Sep 2026.** If the server is set up to keep photos on its own disk without the permanent storage attached, it now **refuses to start** and says so, instead of starting normally and quietly losing every photo the next time it is updated. What is still outstanding is the second half: the setting that tells the server it sits behind the hosting provider's front door. Set wrongly, the system treats all of Cabuyao as one user when limiting password guesses, so one person locks out everybody. That value is set correctly on the live server today, but nothing checks it, so a future change could undo it silently. Specified in `DEPLOYMENT.md` Part 0b.2. | Developer | ☐ |
| c | **Stop the automatic setup step running a second time** | ~~The system re-runs its setup step every time the server starts~~ — **done, 23 Sep 2026.** The server image now only serves; the setup step is no longer attached to start-up, so a restart can no longer overwrite the service deadlines, the barangay coordinates, the category ordering or the office location an administrator has changed. It is still run deliberately, once, when a new server is first set up. | Developer | ☑ |
...
| l | **Remove the demonstration reports from the live system** | The live database was deliberately filled with **49 invented complaints and a set of wildlife turnovers** so the maps and charts had something to show during the capstone defense. They are not real reports of anything. Left in place they are counted in the office's own statistics, printed reports and SLA figures, and there is no way to tell them apart by looking. They were created by a script that also removes them, so this is one command, run once: `npm run seed:demo -- --remove --apply` on the server. Do it before the office relies on any figure the system reports. No identities were invented — reporter names and contacts were left blank — so nothing here is a privacy matter, only an accuracy one. | Developer | ☐ |
